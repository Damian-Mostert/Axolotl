...
std::string Interpreter::visit(FunctionCall *node)
{
    ...
    // Check builtin registry FIRST (before all hardcoded builtins)
    if (auto builtin = BuiltinRegistry::instance().getBuiltin(callName)) {
        return builtin->execute(this, node);
    }
