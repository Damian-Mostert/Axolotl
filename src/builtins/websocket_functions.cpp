...
public:
    ...
    {
        if (node->args.size() != 1)
            throw std::runtime_error("wsConnect(url)");
...
public:
    ...
    std::string execute(Interpreter *interp, FunctionCall *node) override
    {
        if (!node->callee || node->args.size() != 1)
            throw std::runtime_error("ws.send(message)");
        ...
        if (!wsConnections.count(wsId))
            throw std::runtime_error("Invalid WebSocket connection");
...
public:
    ...
    {
        if (!node->callee)
            throw std::runtime_error("ws.receive()");
...
public:
    ...
    {
        if (!node->callee)
            throw std::runtime_error("ws.close()");
        ...
        if (wsConnections.count(wsId))
        {
            auto conn = wsConnections[wsId];
            if (conn->wsi)
                lws_close_reason(conn->wsi, LWS_CLOSE_STATUS_NORMAL, nullptr, 0);
