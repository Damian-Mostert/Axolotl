...
public:
    ...
    {
        if (node->args.size() != 2)
            throw std::runtime_error("createServer() expects 2 arguments");
...
public:
    ...
    std::string execute(Interpreter *interp, FunctionCall *node) override
    {
        ...
        if (node->args.size() < 1 || node->args.size() > 2)
            throw std::runtime_error("writeHead() expects 1 or 2 arguments");
...
public:
    ...
    std::string execute(Interpreter *interp, FunctionCall *node) override
    {
        if (!node->callee)
            return "";
        if (node->args.size() != 1)
            throw std::runtime_error("write() expects 1 argument");
        if (auto fieldAccess = dynamic_cast<FieldAccess *>(node->callee.get()))
        {
            Value resVal = interp->evaluate(fieldAccess->object.get());
            if (!std::holds_alternative<std::shared_ptr<ObjectValue>>(resVal))
...
public:
    ...
    std::string execute(Interpreter *interp, FunctionCall *node) override
    {
        if (!node->callee)
            return "";
        if (node->args.size() > 1)
            throw std::runtime_error("end() expects 0 or 1 argument");
        if (auto fieldAccess = dynamic_cast<FieldAccess *>(node->callee.get()))
        {
            Value resVal = interp->evaluate(fieldAccess->object.get());
            if (!std::holds_alternative<std::shared_ptr<ObjectValue>>(resVal))
...
public:
    ...
    {
        if (node->args.size() < 1 || node->args.size() > 2)
            throw std::runtime_error("fetch() expects 1 or 2 arguments");
        ...
        if (node->args.size() == 2)
        {
            ...
            if (std::holds_alternative<std::shared_ptr<ObjectValue>>(optionsVal))
            {
                ...
                if (options->fields.find("method") != options->fields.end())
                    method = interp->valueToString(options->fields["method"]);
                if (options->fields.find("body") != options->fields.end())
                {
                    ...
                    if (std::holds_alternative<std::shared_ptr<ObjectValue>>(bodyVal))
                    {
                        ...
                        body = interp->valueToString(bodyVal);
                }
                if (options->fields.find("headers") != options->fields.end())
                {
                    ...
                    if (std::holds_alternative<std::shared_ptr<ObjectValue>>(headersVal))
                    {
                        ...
                        for (const auto &[key, val] : headersObj->fields)
                            headers[key] = interp->valueToString(val);
                    }
                }
            }
        }
            ...
            try
            {
                ...
                while (pos < responseBody.size() && responseBody[pos] != '}')
                {
                    while (pos < responseBody.size() && (responseBody[pos] == ' ' || responseBody[pos] == '\n'))
                        pos++;
                    if (pos >= responseBody.size() || responseBody[pos] == '}')
                        break;
                    if (responseBody[pos] != '"')
                        break;
                    pos++;
