...
public:
    ...
    {
        if (node->args.size() != 1)
            throw std::runtime_error("isKeyDown() expects 1 argument: key name");
...
public:
    ...
    {
        if (node->args.size() != 0)
            throw std::runtime_error("getMouseX() expects no arguments");
        int x, y;
...
public:
    ...
    {
        if (node->args.size() != 0)
            throw std::runtime_error("getMouseY() expects no arguments");
        int x, y;
        SDL_GetMouseState(&x, &y);
        interp->lastValue = y;
        return "[int]";
...
public:
    ...
    {
        if (node->args.size() != 0)
            throw std::runtime_error("isMouseDown() expects no arguments");
...
public:
    ...
    {
        if (node->args.size() != 0)
            throw std::runtime_error("wasMouseClicked() expects no arguments");
