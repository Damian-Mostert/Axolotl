...
const BUILTINS: Record<string, string> = {
    ...
    'clear': 'clear() -> void',
    'createLight': 'createLight(x: float, y: float, z: float) -> object',
    'createMaterial': 'createMaterial(color: string) -> object',
    'setIntensity': 'setIntensity(value: float) -> void',
    'setProperties': 'setProperties(ambient: float, diffuse: float, specular: float, shininess: float) -> void'
};

interface Symbol {
    name: string;
    type: string;
    kind: 'var' | 'const' | 'func' | 'type';
    line: number;
}

function parseFile(text: string): Symbol[] {
    ...
    lines.forEach((line, i) => {
        ...
        if (varMatch) {
            symbols.push({name: varMatch[2], type: varMatch[3].trim(), kind: varMatch[1] as 'var'|'const', line: i});
        }
...
export function activate(context: vscode.ExtensionContext) {
    context.subscriptions.push(
        vscode.languages.registerCompletionItemProvider('axolotl', {
